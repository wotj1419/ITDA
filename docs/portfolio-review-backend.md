...
> 아래 항목은 **이미 문서에 언급된 내용과 일부 중복**될 수 있으나, 보완 관점/지표까지 포함해 다시 정리한 추가 목록입니다.

### 1) 비밀번호 재설정 흐름 강화 (토큰 기반 + 남용 방지)

- **개선 방향:** 이메일만으로 재설정하지 않고, **1회용 토큰 기반**으로 전환 (Redis 저장, TTL 10~15분).
- **남용 방지:** 이메일/아이피 기준 **rate limit** + 요청 빈도 이상 탐지.
- **지표:** 재설정 요청 성공률, 실패율, 동일 이메일/아이피 반복 요청 탐지 비율.

---

### 2) 테스트 엔드포인트 제한

- **개선 방향:** `/test/**`는 `local/test` 프로필에만 활성화.
- **운영 차단:** SecurityConfig에서 permitAll 제거, 프로파일 미활성화 시 Bean 등록 자체 차단.
- **지표:** 운영 환경 test API 접근 차단 성공률 100%.

---

### 3) 씬 헤더 생성 부작용 제거

- **개선 방향:** 조회 API에서 생성하지 말고 **Scene 생성 트랜잭션에서 헤더 노드를 같이 생성**.
- **무결성 강화:** `(scene_id, node_type)` **유니크 제약** 추가로 중복 생성 방지.
- **지표:** GET API의 side-effect 제거, 중복 헤더 발생률 0%.
...
- **Job 조회 최적화 (예시):**
  ...
  - `generation_jobs(scene_id, status)`
  - `generation_jobs(project_id, status)`
- **Node 조회 최적화 (예시):**
  - `nodes(scene_id, node_type)`
  - `nodes(scene_id, status)`
- **지표:** 특정 조회쿼리 p95 응답시간 50% 이상 감소.

---

### 5) 캐싱 적용 (Spring Cache + Redis)

- **대상:** 프로젝트 상세, 노드 목록, 프로젝트 목록
- **전략:** 조회는 `@Cacheable`, 변경은 `@CacheEvict`
- **지표:** 캐시 히트율 70%+, 응답시간 5~10배 개선

---

### 6) 쿼리 최적화 + 배치 업데이트

- 반복 조회되는 N+1 쿼리를 **JOIN/IN 조건**으로 축소
- 노드 위치 저장 등 **배치 update**로 round-trip 감소
- **지표:** 동일 요청의 DB 호출 횟수 30% 이상 감소

---

### 7) 페이지네이션 개선 (Cursor 기반)

- Offset 기반 대신 `last_id` 기반 커서 적용
- **지표:** 1000페이지 조회에서도 응답시간 일정 유지

---

### 8) 낙관적 락킹 도입 확대

- Job 외에 Node/Scene 업데이트에도 `version` or `updated_at` 기반 낙관적 락 추가
- **지표:** 동시 업데이트 충돌 시 데이터 정합성 보장

---

### 9) 아이디엠포턴시 보장 강화

- 현재 unique key 활용 여부 점검
- `idempotency_key`가 null이어도 중복 허용되지 않게 정책 강화 (자동 생성 or NOT NULL)
- **지표:** 중복 요청 발생 시 동일 Job 반환률 100%

---

### 10) AOP 로깅 도입

- 모든 API에 **requestId + userId + 응답시간** 자동 기록
- Slow API 임계치 초과 시 WARN 분리
- **지표:** 장애 시 요청 단위 추적 가능

---

### 11) 데드레터 큐(DLQ) 시스템 구축

- Redis Streams 실패 메시지 → **DLQ로 이동**
- 성공 시만 ACK 처리, 실패 횟수 초과 시 DLQ 저장
- **지표:** 장애 발생 시 유실률 0%, 재처리 가능
