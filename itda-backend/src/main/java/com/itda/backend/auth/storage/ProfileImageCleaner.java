...
public class ProfileImageCleaner {
    ...
    public void delete(StorageProvider storageProvider, String storageKey) {
        ...
        try {
            ...
            log.warn("[ProfileImageCleaner] Failed to delete profile image: provider={}, key={}", storageProvider, storageKey, e);
        }
    }
    ...
    private void deleteFromS3(String storageKey) {
        ...
        if (s3Client == null) {
            log.warn("[ProfileImageCleaner] S3 client not available. skip delete: key={}", storageKey);
            return;
        }
        String bucket = s3Properties.getBucket();
        if (bucket == null || bucket.trim().isEmpty()) {
            log.warn("[ProfileImageCleaner] S3 bucket not configured. skip delete: key={}", storageKey);
            return;
        }
